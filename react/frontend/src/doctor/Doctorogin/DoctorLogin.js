...
export const DoctorLogin = () => {
    ...
    const handleLogin = async () => {
        ...
        if (password.length < 5) {
            changeError("PASSWORD SHOULD BE AT LEAST 5 CHARACTERS");
            setTimeout(hideError, 3000);
            return;
        }
  // getting the data from db using api and setting them as eamil,pass,role
        try {
            const response = await axios.post("http://localhost:1111/officialEmail/", {
                email: email,
                password: password,
                role: "Doctor"
            });
            ...
            if (userData.role === "Doctor" && userData.email === email && userData.password === password) {
                navigate("/Doctorhome");
            } else {
                changeError("INVALID USER");
                setTimeout(hideError, 3000);
